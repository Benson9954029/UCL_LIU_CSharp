...
C# / WinForms 版肥米輸入法，目前版本 v0.15。這個分支的目標是保留 2019 年 C# 版輕量、低依賴、好攜帶的優點，同時逐步追上 Python 版 UCL_LIU 後來累積的穩定性與工具鏈。
...
| C# 版 v0.15 | 8.3 / 10 | 8.3 / 10 | 8.2 / 10 | 同音/注音、TSF 管理與半全形快捷鍵更完整，低依賴、可維護的現代化復刻版 |
...
C# 版 v0.15 目前已整理成「無外部 NuGet、無 DLL 合併」的乾淨 WinForms 專案。主程式只使用 .NET Framework 4.5.2 內建 reference，功能邏輯盡量收在專案原始碼裡，方便直接用 Visual Studio 或 MSBuild 重建。TSF Bridge 的 C++ 原始碼也已納入 `tsf_bridge\UclTsfBridge`，可由本 repo 自行重建。
...
| 檔案 | 說明 |
| --- | --- |
| [uclliu-v0.15.zip](https://github.com/shadowjohn/UCL_LIU_CSharp/releases/download/v0.15/uclliu-v0.15.zip) | v0.15 推薦下載包，含 `uclliu.exe`、`pinyi.txt`、`tsf_bridge`、README 與 LICENSE |
| [uclliu.exe](https://github.com/shadowjohn/UCL_LIU_CSharp/releases/download/v0.15/uclliu.exe) | v0.15 單檔版，不含 TSF Bridge、同音/注音資料與音效素材 |
...
v0.15 發行日期：2026-06-04，發行檔由 GitHub Actions 自動測試、編譯 TSF Bridge 與打包。v0.12 病毒碼掃描已提交 Microsoft WDSI：[submission a42546c1-4432-40f2-8cc6-6e226617cf19](https://www.microsoft.com/en-us/wdsi/submission/a42546c1-4432-40f2-8cc6-6e226617cf19)。
...
v0.15 推薦下載包 `uclliu-v0.15.zip` 內含：
...
v0.15 推薦下載包已內含 `pinyi.txt`；若只下載單檔 `uclliu.exe`，需要自行補上 `pinyi.txt` 才會有同音字與注音查詢。
...
TSF Bridge 不會自動啟用，也不會自動註冊。要使用 TSF，請下載 `uclliu-v0.15.zip` 並完整解壓縮，讓 `tsf_bridge` 目錄與 `uclliu.exe` 放在同一層。只下載單檔 `uclliu.exe` 時不會包含 TSF Bridge。
...
1. 下載並解壓縮 [uclliu-v0.15.zip](https://github.com/shadowjohn/UCL_LIU_CSharp/releases/download/v0.15/uclliu-v0.15.zip)。
2. 執行 `uclliu.exe`。
...
```powershell
& .\tools\package-release.ps1 -Version v0.15 -Configuration Release -OutputDirectory .\artifacts
```
...
自動 Release 會上傳兩個檔案：`uclliu-v0.15.zip` 是推薦下載包，含 TSF Bridge 與同音/注音資料；`uclliu.exe` 是單檔版，不含 TSF Bridge。
